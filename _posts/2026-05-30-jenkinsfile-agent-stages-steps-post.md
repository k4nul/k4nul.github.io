---
layout: single
title: "Jenkins 06. Jenkinsfile 읽기: agent, stages, steps, post를 어떻게 구분할까"
description: "Jenkinsfile을 처음 읽는 사람이 agent, stages, steps, post 블록을 실행 흐름 기준으로 구분하는 방법을 정리한 글."
...
Jenkinsfile을 처음 열면 `pipeline { ... }` 안에서 `agent`, `stages`, `steps`, `post`가 반복해서 보인다. 이 네 블록은 Groovy 문법 암기 대상이라기보다 "어디서 실행하고, 어떤 순서로 나누고, 각 단계에서 무엇을 하고, 끝난 뒤 무엇을 남길지"를 읽는 운영 구조다.

이 글의 결론은 Jenkinsfile을 처음부터 Groovy 코드처럼 보지 말고 실행 계획표로 읽어야 한다는 것이다. [Jenkins 05. Declarative Pipeline 입문](/devops/jenkins-declarative-pipeline-introduction/) 다음 단계에서 `agent`, `stages`, `steps`, `post`를 구분하면 이후 Docker build나 배포 단계도 더 쉽게 읽힌다.
...
특히 `agent any`를 복사해 붙여 넣거나 `steps { sh '...' }`만 반복하는 상태에서는 "왜 이 stage가 여기서 실행되는지"와 "실패 후 어떤 정리 작업이 남아야 하는지"가 잘 보이지 않는다. 이번 글은 그 흐름을 읽는 최소 기준을 정리한다.
...
- [DevOps 운영 흐름](/devops/)
- [Jenkins 05. Declarative Pipeline 입문](/devops/jenkins-declarative-pipeline-introduction/)
