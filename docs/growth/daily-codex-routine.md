...
Codex는 매일 K4NUL 블로그를 `AI coding agent 운영·검증·보안` 한국어 실무형 지식베이스로 관리한다. 기본 작업은 신규 발행이 아니라 구조화, 리라이트, 내부링크, 템플릿화, 검증 기록이다.
...
2. 오늘의 작업 유형을 요일표에서 고른다.
3. 변경 범위를 1-3개 파일 또는 1개 허브/글 묶음으로 제한한다.
4. title, description, 첫 문단, TL;DR, 내부링크, 허브 링크, 템플릿 링크를 점검한다.
...
| 요일 | 주요 작업 | 완료 기준 |
| --- | --- | --- |
| 월요일 | 신규 long-tail 검색 글 1개 작성 또는 초안 작성. 주제는 `AGENTS.md`, `CLAUDE.md`, Codex, Claude Code, hooks, MCP, token/context 중 하나. | 새 글 1개 이하 또는 초안 1개, 허브 연결 후보 기록, build 결과 기록 |
| 화요일 | 기존 글 1-2개 리라이트. 제목, description, 첫 문단, 내부링크 개선. | URL 보존, 변경 전후 의도 기록, 관련 글 3개 이상 후보 확인 |
| 수요일 | 템플릿/체크리스트 페이지 1개 작성 또는 보강. 예: AGENTS.md 템플릿, CLAUDE.md 템플릿, hooks 예제, MCP 보안 체크리스트. | 템플릿 적용 조건, 금지 사항, 검증 방법 포함 |
| 목요일 | 내부링크 정리. 허브 -> 개별 글, 개별 글 -> 허브, 개별 글 -> 템플릿, 관련 글 3개 연결. | 약한 앵커 텍스트 제거, 링크 존재 확인 |
| 금요일 | 실험/postmortem 글 작성 또는 보강. 실제 재현, 실패 사례, 검증 기준, 한계를 명시. | 사실/관찰/해석/한계 분리, 검증 기준일 기록 |
| 토요일 | `/ai-engineering/`, `/start-here/`, `/ai-engineering/templates/` 허브 보강. 새로 생긴 글을 허브에 연결. | 허브와 템플릿 경로 유지, canonical 영향 없음 |
| 일요일 | Search Console/GA4 데이터가 제공된 경우 분석. 데이터가 없으면 다음 주 작업 후보만 정리. 제목 수정 후보, 리라이트 후보, 영어화 후보 정리. | 수치 추정 없음, 사용자 입력 필요 값 명시 |
...
- 전체 제목 대량 변경, 영어화, cannibalization 정리는 데이터와 별도 프롬프트 없이 즉시 실행하지 않는다.
