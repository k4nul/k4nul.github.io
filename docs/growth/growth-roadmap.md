...
- 새 글은 1개 이하로 제한하고, 리라이트와 내부링크 개선을 우선한다.
...
- long-tail 검색 글을 작성하거나 기존 글을 long-tail 의도에 맞게 리라이트한다.
...
- Rust, DevOps, Security 글 중 agent 검증/보안과 자연스럽게 연결되는 글만 내부링크로 묶는다.

## Months 4-6

- Search Console에서 성과가 확인된 템플릿과 허브만 영어화한다.
- 영어화 범위는 reference, template, how-to 페이지 중심으로 제한한다.
- 전체 번역 대신 검색 수요가 확인된 페이지부터 영문 mirror 또는 영문 reference를 만든다.
